  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#fdf6ee] via-[#fff7ed] to-[#eff6ff] flex flex-col">
      ...
      <nav className="flex items-center justify-between px-8 py-5">
        <span className="font-black text-xl tracking-tight text-foreground">LearnSmarter</span>
        <button
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-6 pt-8 pb-16 gap-8">
        <div className="max-w-2xl">
          ...
          <h1 className="text-5xl md:text-6xl font-black leading-tight mb-4 text-foreground">
            Learn Smarter,{" "}
            <span className="text-primary">Compete Better</span>
          </h1>
          <p className="text-lg text-muted-foreground max-w-xl mx-auto leading-relaxed">
            Timed practice tests for olympiad and competitive exam preparation.
            Parents track every detail with Bloom's Taxonomy insights.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button
            onClick={() => setLocation("/auth")}
            className="flex items-center gap-2 bg-primary text-white px-8 py-4 rounded-2xl text-lg font-bold hover:opacity-90 transition-opacity shadow-lg shadow-orange-200"
          >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 max-w-3xl w-full">
          {[
            ...
            { icon: Trophy, label: "Progress Tracking", desc: "Track improvement", color: "bg-yellow-100 text-yellow-600" },
          ].map(({ icon: Icon, label, desc, color }) => (
            <div key={label} className="bg-white rounded-2xl p-4 border text-left">
